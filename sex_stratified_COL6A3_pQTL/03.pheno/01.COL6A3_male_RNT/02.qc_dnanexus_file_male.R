...
setwd('/scratch/richards/satoshi.yoshiji/30.pQTL_regenie/03.pheno/01.COL6A3_male_RNT/')
...
# pheno %<>% mutate(Sex = ifelse(Sex == 'male', 0, 1)) %>% mutate(Genetic_sex = ifelse(Genetic_sex == 'male', 0, 1)) # male = 0 , Male = 1
# 
...
combine_unique <- fread('/scratch/richards/satoshi.yoshiji/30.pQTL_regenie/03.pheno/01.COL6A3_RNT/02.pheno_covariate.tsv') # Note that this is normalized yet

# Sex = 0 (male) only
combine_unique_male <- combine_unique %>% filter(Sex == 1)
nrow(combine_unique_male) # 16925

# RNT¬
combine_unique_male$Protein_level <- RNOmni::RankNorm(combine_unique_male$Protein_level)

# save
write_tsv(combine_unique_male, file = '02.pheno_covariate_male.tsv')
write_tsv(combine_unique_male %>% select(FID, IID, Protein_level) , file = '02.pheno_male.tsv')
write_tsv(combine_unique_male %>% select(-Protein_level) , file = '02.covariate_male.tsv')

# save individuals to be kept for step 1 and 2 analyses
keep_male0 <- combine_unique_male %>% select(FID, IID) 
...
`%!in%` <- Negate(`%in%`)
keep_male_excluded <- keep_male0 %>% filter(!IID %in% exclude$V1) 

# write
write_tsv(keep_male_excluded, file = '02.keep_male.tsv', col_names = F)
